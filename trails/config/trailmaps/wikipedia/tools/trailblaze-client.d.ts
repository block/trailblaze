...
declare module "@trailblaze/scripting" {
  interface TrailblazeToolMap {
     ...
     * Maestro `launchApp({ launchMode: FORCE_RESTART })` against `org.wikipedia`, but
     * composed entirely from the dual-mode `android_adbShell` primitive so the same tool works
     * on host- and on-device-dispatched scripted-tool sessions.
     * 
