...
fun YamlTabComposable(
  ...
  databricksOauthProvider: (suspend (allowAuthFlow: Boolean) -> String?),
) {
                ...
                yamlRunner(
                  DesktopAppRunYamlParams(
                    ...
                    additionalInstrumentationArgs = {
                      val databricksToken = databricksOauthProvider.invoke(true)
                      if (databricksToken.isNullOrEmpty()) {
                        emptyMap()
                      } else {
                        mapOf("DATABRICKS_TOKEN" to databricksToken)
                      }
                    }
                  )
                )
