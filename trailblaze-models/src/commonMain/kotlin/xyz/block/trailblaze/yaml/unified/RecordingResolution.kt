...
data class RecordingResolution(
  ...
  companion object {
     ...
     *
     * **A floor, not a complete set.** Membership is by name, and this list only names framework
     * tools — a target's own self-guarding tools cannot be enumerated here even when they meet the
...
data class TrailRecordingResolution(
   ...
   * not the shape of the recordings. Every candidate examined so far turned out to be legitimate
   * divergence rather than a defect. Use this to *list* candidates for a human to read; do not
   * gate on it.
   *
