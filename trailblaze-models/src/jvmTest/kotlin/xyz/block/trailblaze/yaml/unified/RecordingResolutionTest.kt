...
class RecordingResolutionTest {
  ...
  fun `a matched-empty step is not reported as a lost guard`() {
    ...
    // reconciling two independent counts over the same trails: including these over-reported the
    // device cells, and the whole residual disagreement between the two counts WAS those cells —
    // the null/empty conflation this file exists to prevent, one level up.
    val unified = UnifiedTrail(
      config = UnifiedTrailConfig(id = "x", target = "y"),
      trail = listOf(
        UnifiedTrailStep(
